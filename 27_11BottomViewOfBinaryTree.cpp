...
void Topview(Node * root)
{
     ...
     while(!Q.empty())
     {
        ...
        m[currhd]=currNode->data;/*Only this thing is different in bottom view approch as compares to top view approach.
        if the same key is come's then we just upgrate the value so its gives a bottom view logic.
        IN top view if the same key comes then we don't upgrate the value we just ignored, so thats why its gives a top view approach
*/
